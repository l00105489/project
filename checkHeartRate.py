...
def parse_command_line_args():
    ...
    parser.add_argument(
            '--cloud_region', default='europe_west2', help='GCP cloud region')
    parser.add_argument(
            '--ca_certs',
...
def createJSON(pir, mat, mic, timestamp):
    data = {
    'PIR' : pir,
    'MAT' : mat,
    'MIC' : mic,
...
#            uniqueID = str(uuid.uuid4()) + "-" + sensorID
                        pir = sensordata.pirSensor()
                        mat = sensordata.matSensor()
                        mic = sensordata.micSensor()
                        
                        payload = createJSON(pir, mat, mic, currentTime)
